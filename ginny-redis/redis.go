...
func NewRedis(ctx context.Context, config *Config, logger *zap.Logger) (*Redis, error) {
	...
	} else if config.Standalone.Master != "" {
...
// Cluster集群或者独立部署了多主分片节点：采用NewClusterClient()函数 ClusterSlots配置 返回ClusterClient
...
func clusterOptions(config *Config) *redis.ClusterOptions {
	...
	// 自建多主分片集群，或有主从读写分离配置，采用ClusterSlots配置 返回ClusterClient
	opt.ClusterSlots = func(ctx context.Context) ([]redis.ClusterSlot, error) {
		if len(config.ClusterSlotAddrs) <= 0 {
			return nil, errors.New("empty cluster slots")
		}
		slots := make([]redis.ClusterSlot, 0, len(config.ClusterSlotAddrs))
		slotInterval := 16384 / len(config.ClusterSlotAddrs)
		for i := 0; i < len(config.ClusterSlotAddrs); i++ {
			nodes := make([]redis.ClusterNode, 0, len(config.ClusterSlotAddrs[i].Slaves)+1)
			nodes = append(nodes, redis.ClusterNode{Addr: config.ClusterSlotAddrs[i].Master})
			for _, slave := range config.ClusterSlotAddrs[i].Slaves {
			...
			if i == len(config.ClusterSlotAddrs)-1 {
				slot.End = 16383
			}

...
func parseEndpoint(url *url.URL) (serviceName, serviceScheme string, tags []string, health healthFilter, token string, err error) {
	...
	serviceScheme, tags, health, token, err = extractOpts(url.Query())
	if err != nil {
	...
	return serviceName, serviceScheme, tags, health, token, nil
}
